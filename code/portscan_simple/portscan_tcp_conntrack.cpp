...
namespace net::tcp {
  ...
  static inline Ct_state state(PortscanConntrack::Entry* entry);
  [[maybe_unused]]
  static inline std::string state_str(const uint8_t state);

  static inline void update_timeout(PortscanConntrack::Entry* entry);
  static inline void set_state(PortscanConntrack::Entry* entry, const Ct_state state);
  static inline void close(PortscanConntrack::Entry* entry);

  PortscanConntrack::Entry* portscan_tcp4_conntrack(PortscanConntrack& ct, Quadruple q, const PacketIP4& pkt)
  {
    using State = PortscanConntrack::State;
    ...
    if(UNLIKELY(entry == nullptr))
    {
      ...
      {
        if(ct.isopen(q.dst)) {
          ct.handle_flood_packet(q, PortscanConntrack::PS_type::SYN_FLOOD);
        }
        if(UNLIKELY(tcp.isset(ACK)))
        {
          ct.handle_flood_packet(q, PortscanConntrack::PS_type::ACK_FLOOD);
          //We missed or got rid of the entry for the initial SYN
          return nullptr;
        }
        // this was a SYN, set internal state SYN_SENT & UNREPLIED
        entry = ct.add_entry(q, proto);
        entry->set_flag(PortscanConntrack::Flag::UNREPLIED);
      ...
      if((tcp.isset(FIN) and tcp.isset(URG) and tcp.isset(PSH))) 
        ct.handle_scan_packet(q, PortscanConntrack::PS_type::XMAS_SCAN);
      else if(tcp.isset(FIN))
        ct.handle_scan_packet(q, PortscanConntrack::PS_type::FIN_SCAN);
      else if(not tcp.isset(FIN) and not tcp.isset(ACK) and not tcp.isset(PSH) and not tcp.isset(URG) and not tcp.isset(RST))
        ct.handle_scan_packet(q, PortscanConntrack::PS_type::NULL_SCAN);
      }
    ...
    {
      if(tcp.isset(ACK) and state(entry) == Ct_state::SYN_SENT and q == entry->second)
        ct.handle_scan_packet(entry->first, PortscanConntrack::PS_type::SYN_CLOSED);
      close(entry);
      return entry;
    }
    ...
    if(UNLIKELY(state(entry) == Ct_state::SYN_SENT and q == entry->second))
    {
      ...
      {
        entry->unset_flag(PortscanConntrack::Flag::UNREPLIED);
    ...
    if(UNLIKELY(state(entry) == Ct_state::SYN_RECV and q == entry->first))
    {
      if(tcp.isset(ACK))
      {
        ...
        entry->set_flag(PortscanConntrack::Flag::ASSURED);
  ...
  static inline void update_timeout(PortscanConntrack::Entry* entry)
  {
    ...
    entry->timeout = RTC::nanos_now()/1000000000ull + dur.count();
  }
